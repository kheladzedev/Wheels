...
def _write_eval(path: Path, *, map50: float, oks: float, fn: float) -> None:
    path.write_text(
        json.dumps(
            {
                "metrics_bbox": {"mAP50": map50},
                "oks": {"mean": oks},
                "rates": {"false_negative_rate": fn},
            }
        ),
        encoding="utf-8",
    )
...
def test_integration_gate_ignores_production_only_blockers(tmp_path, monkeypatch):
    ...
    (tmp_path / "docs" / "PRODUCTION_READINESS_AUDIT.md").write_text("audit", encoding="utf-8")
    pt = tmp_path / "best.pt"
    ...
    _write_eval(real_eval, map50=0.9, oks=0.88, fn=0.05)
    _write_eval(anchor_eval, map50=0.7, oks=0.88, fn=0.2)
    _write_eval(onnx_eval, map50=0.69, oks=0.88, fn=0.2)
    drift.write_text('{"ok": false, "samples_matched": 14, "samples_checked": 20}', encoding="utf-8")
    ...
    assert any(item.name == "onnx_strict_parity_diagnostic" and not item.ok for item in items)
...
def test_tflite_gate_requires_certified_true(tmp_path):
    missing = pg.certification_gate("tflite_litert_certified", tmp_path / "missing.json")
...
def test_ar_report_gate_requires_ok_true(tmp_path):
    ...
    failed.write_text(json.dumps({"ok": False, "failures": ["too_few_observations"]}), encoding="utf-8")
