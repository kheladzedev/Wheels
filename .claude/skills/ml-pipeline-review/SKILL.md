...
description: Use before claiming an ML / dataset / inference task is "done" in VSBL. Activates whenever Claude is about to summarise completed work, write the final report for a /goal, or open a PR. Forces a verification pass — running tests, listing changed files, distinguishing synthetic-data plumbing from real quality signal — so we never ship a green-pytest-on-cartoons as "production ready".
---
...
grep -nE "rim_left|rim_right|metal rim left|metal rim right|left point of metal rim|right point of metal rim" \
    docs/ README.md CLAUDE.md .claude/skills/ src/manual_keypoint_annotator.py \
