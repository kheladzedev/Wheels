...
def _metric(report: dict[str, Any], key: str, default: float | None = None) -> float | None:
    try:
        value = report[key]
    except KeyError:
        return default
...
def _find_eval(
    ...
    *,
    ...
    candidates = [
        ...
        if _norm_path(report.get("model")) == model and _norm_path(report.get("data")) == data
    ]
    if not candidates:
        return None
    return max(
...
def build_audit(
    ...
    *,
    ...
    for report in sorted(anchor_reports, key=lambda item: _norm_path(item.get("model"))):
    ...
    if champion_anchor is not None and not any(c["model"] == champion for c in candidates):
    ...
    return {
        ...
        "champion_evidence": {
            "anchor_eval": _norm_path(champion_anchor.get("path")) if champion_anchor else None,
            "real_eval": _norm_path(champion_real.get("path")) if champion_real else None,
